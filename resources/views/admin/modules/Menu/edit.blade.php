                    ...
                    <form action="{{ route('menus.update', $menu->id) }}" method="POST">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <input type="text" name="location" class="form-control" value="{{ old('location', $menu->location) }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body bg-light" id="menuItemsContainer">
                                ...
                                @foreach($menu->allItems as $index => $item)
                                    <div class="card mb-2 menu-item border-secondary">
                                        <div class="card-body p-3">
                                            <div class="row align-items-end">
                                                <div class="col-md-3">
                                                    <label>Link Title <span class="text-danger">*</span></label>
                                                    <input type="text" name="items[{{ $index }}][title]" class="form-control" required value="{{ $item->title }}">
                                                </div>
                                                <div class="col-md-4">
                                                    <label>Link to Dynamic Page</label>
                                                    <select name="items[{{ $index }}][page_id]" class="form-select page-select">
                                                        <option value="">-- Custom URL --</option>
                                                        @foreach($pages as $p)
                                                            <option value="{{ $p->id }}" {{ $item->page_id == $p->id ? 'selected' : '' }}>{{ $p->title }} (/{{ $p->slug }})</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-4">
                                                    <label>Or Custom URL</label>
                                                    <input type="text" name="items[{{ $index }}][url]" class="form-control custom-url-input" value="{{ $item->url }}" {{ $item->page_id ? 'readonly' : '' }}>
                                                </div>
                                                <div class="col-md-1">
                        ...
                        <button type="submit" class="btn btn-primary btn-lg w-100">Update Menu</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function () {
        let itemIndex = {{ $menu->allItems->count() > 0 ? $menu->allItems->count() : 0 }};
